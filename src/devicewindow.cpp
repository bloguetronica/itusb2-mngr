...
void DeviceWindow::update()
{
    ...
    bool cd = device_.getGPIO4(errcnt, errstr);  // Get the current value of the GPIO.4 pin, which corresponds to the UDCD signal;
    bool hs = device_.getGPIO5(errcnt, errstr);  // Get the current value of the GPIO.5 pin, which corresponds to the UDHS signal;
...
{
    timer_->stop();  // Stop the update timer momentarily, in order to avoid a segmentation fault if the device gets disconnected during a reset, or other unexpected behavior
    clearMetrics();
