...
{
    QString csvstr = "Time (s),Current (mA),USB power,USB data,Device detected,Link mode,OC flag\n";
    for (DataPoint datapt : dataPoints_) {
        ...
        csvstr.append(QString("%1,").arg(datapt.cd));
        csvstr.append(QString("%1,").arg(datapt.hs));
